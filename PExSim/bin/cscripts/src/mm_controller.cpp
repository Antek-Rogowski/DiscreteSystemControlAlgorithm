...
std::map<int, double> gTime, stopTime, uIntegrate;
...
/*  if (_in_val[0] <= 0 || _in_val[1] <= 0 || _in_val[2] <= 0) {
  ...
  if (t < 0) {
    ...
    if (_in_val[6] < _in_val[1]+2*ds) {
      if (stopTime[_block_id] == 0) {
        stopTime[_block_id] = t;
        uIntegrate[_block_id] = _in_val[3]  - k * (_in_val[6]-_in_val[1]);
        _out_val[0] = _in_val[3];
      } else {
        // PI action:

        double e = _in_val[6]-_in_val[1];
        double b1i = k * _sample_rate/Ti;

        _out_val[0] = k*e + uIntegrate[_block_id] + b1i*e;
      }
  ...
  _out_val[1] = stopTime[_block_id] > 0;
  _out_status[0] = _out_status[1] = 0;
...
extern "C" DLLIMPORT int csSetIOName(const int _block_id, int _type, int _index, char* _name, const int _name_size)
{
  if (_type == 0)
  {
    switch (_index) {
      ...
      case 7 : strcpy(_name, "k"); break;
      case 8 : strcpy(_name, "Ti"); break;
    }
  } else if (_type == 1) {
    switch (_index) {
      case 0 : strcpy(_name, "v"); break;
      case 1 : strcpy(_name, "mode"); break;
    }
  }
  return 0;
}
