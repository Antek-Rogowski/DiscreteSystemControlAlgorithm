                            ...
                            const int _out_size)
{
  ...
  double t = gTime[_block_id];
  double ta = _in_val[1]/_in_val[2];
  double tm = (_in_val[0]-_in_val[1]*_in_val[1]/_in_val[2]) / _in_val[1];
  
  if (t < _in_val[3]) {
    _out_val[0] = 0;
  } else if (t < ta+_in_val[3]) {
    _out_val[0] = _in_val[2] * (t-_in_val[3]);
  } else if (t < ta+tm+_in_val[3]) {
    _out_val[0] = _in_val[1];
  } else if (t < 2*ta+tm+_in_val[3]) {
    _out_val[0] = _in_val[1] - _in_val[2]*(t-ta-tm-_in_val[3]);
  } else {
    _out_val[0] = 0.0;
  }
...
extern "C" DLLIMPORT int csSetIOName(const int _block_id, int _type, int _index, char* _name, const int _name_size)
{
  ...
  {
    switch (_index) {
      case 0 : strcpy(_name, "s"); break;
      case 1 : strcpy(_name, "v_max"); break;
      case 2 : strcpy(_name, "a"); break;
      case 3 : strcpy(_name, "t0"); break;
    }
  } else if (_type == 1) {
    switch (_index) {
      case 0 : strcpy(_name, "v"); break;
    }
  }
  return 0;
}
