...
extern "C" DLLIMPORT int csCalculate(const int _block_id, 
                            ...
                            const int _out_size)
{
  if (_in_val[0] < _in_val[1] && !gContact[_block_id]) {
    gContact[_block_id] = true;
    _out_val[0] = _in_val[2];
  } else {
    _out_val[0] = 0.0;
  }
  _out_status[0] = 0;
...
extern "C" DLLIMPORT int csSetIOName(const int _block_id, int _type, int _index, char* _name, const int _name_size)
{
  ...
  {
    switch (_index) {
      case 0 : strcpy(_name, "pos"); break;
      case 1 : strcpy(_name, "pos_ref"); break;
      case 2 : strcpy(_name, "v"); break;
    }
  } else if (_type == 1) {
    switch (_index) {
      case 0 : strcpy(_name, "v_c"); break;
    }
  }
  return 0;
}
